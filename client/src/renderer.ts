...
import type { Bullet, Tank } from "./physics";
import { BULLET_RADIUS, TANK_HEIGHT, TANK_WIDTH } from "./physics";
...
export class Renderer {
	...
	drawHUD(tanks: Tank[]) {
		...
		for (let i = 0; i < tanks.length; i++) {
			...
			ctx.fillText(
				`P${tank.id + 1}  Bullets: ${MAX_BULLETS_PER_TANK_DISPLAY - tank.bulletCount}/${MAX_BULLETS_PER_TANK_DISPLAY}`,
				28,
				y,
			);
		}
