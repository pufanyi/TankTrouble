...
import type { GameMap, Wall } from "./map";
...
export class PhysicsWorld {
  ...
  private buildWallSegments() {
    for (const wall of this.map.walls) {
      ...
      this.segments.push({
        x1: wall.x1 + nx * ht, y1: wall.y1 + ny * ht,
        x2: wall.x2 + nx * ht, y2: wall.y2 + ny * ht,
        nx, ny,
      });
      ...
      this.segments.push({
        x1: wall.x1 + nx * ht, y1: wall.y1 + ny * ht,
        x2: wall.x1 - nx * ht, y2: wall.y1 - ny * ht,
        nx: -dx / len, ny: -dy / len,
      });
      // Cap at (x2, y2)
      this.segments.push({
        x1: wall.x2 + nx * ht, y1: wall.y2 + ny * ht,
        x2: wall.x2 - nx * ht, y2: wall.y2 - ny * ht,
        nx: dx / len, ny: dy / len,
      });
    }
  ...
  addTank(id: number) {
    ...
    this.tanks.push({
      id, x: sx, y: sy, rotation: 0,
      vx: 0, vy: 0, alive: true, bulletCount: 0,
  ...
  step(inputs: Map<number, TankInput>, dt: number): GameEvent[] {
    ...
    for (const tank of this.tanks) {
      ...
      if (input.fire && tank.bulletCount < MAX_BULLETS_PER_TANK) {
        ...
        this.bullets.push({
          id: bulletId, x: bx, y: by, vx: bvx, vy: bvy,
          owner: tank.id, bounces: 0, alive: true,
        });
        tank.bulletCount++;
  ...
  private bulletWallCollision(
    ...
    if (closestT <= 1) {
      ...
      const scale = speed > 0.01 ? BULLET_SPEED / speed * PHYSICS_TIMESTEP : 0;
      return {
